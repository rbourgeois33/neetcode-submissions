...
public:
    vector<int> topKFrequent(vector<int>& nums, int k) {
        ...
        //std::nth_element(first, nth, last, comp) rearranges the range so that the element at position nth is the one that would be there in a fully sorted range, and it establishes a partition around it. Cheaper than full sort
        std::nth_element(histogram_v.begin(),
                         histogram_v.end()-k,
                         histogram_v.end(),
                         [](_pair_ref_t a, _pair_ref_t b){
                            return a.second<b.second;
